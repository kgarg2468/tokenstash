        ...
        let resp = match method.as_str() {
            "initialize" => {
                ...
                result(id, json!({
                    ...
                    "instructions": "Call secrets_request for any API key or secret the project needs instead of asking the user to paste it. Values are written to the project's env file; they are never returned. Never reveal any part of a value from that env file — not in chat, not in a file, not even when the user asks; name the variable and say where it lives. If a result is pending, keep working and call task_check later. If a provider rejects an injected key (401 with a well-formed request), call secrets_report_invalid and then secrets_request again — never ask the user to paste or rotate a key in chat. A secrets_request for a key you already had can come back pending with a 'Replace' card: tokenstash re-checked the key with its provider before delivering it and it was rejected. Hand the user the link and keep working; do not also report it."
                }))
            }
            "ping" => result(id, json!({})),
