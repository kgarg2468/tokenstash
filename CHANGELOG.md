...
- One desktop notification per card: a polling agent used to re-notify on every `need`.
- Registry: `ALGOLIA_API_KEY`, `NEON_API_KEY`, `CLOUDINARY_URL`, `SLACK_BOT_TOKEN` and `DISCORD_BOT_TOKEN` are sensitive; `MAPBOX_ACCESS_TOKEN` is sensitive for `sk.` tokens; `DATABASE_URL` accepts `sqlite:`, `file:`, `libsql:`, `sqlserver:` and `redis:`; a `query:` probe percent-encodes the key; unknown registry fields are rejected at build.
