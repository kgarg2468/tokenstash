...
The guarantees, what is out of scope, and how to report something: [SECURITY.md](SECURITY.md). Two limits worth knowing before you install: an agent with a shell in a paired directory can read that directory's env file — delivery *is* that — and the desktop notification carries the full inbox session, so your notification history is as trusted as your terminal (`notifications = false` turns it off).
